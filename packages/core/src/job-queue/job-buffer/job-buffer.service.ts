...
export class JobBufferService {
  ...
  addBuffer(buffer: JobBuffer<any>) {
    ...
    if (idAlreadyExists) {
      throw new InternalServerError(`There is already a JobBufferProcessor with the id "${buffer.id}". Ids must be unique`);
    }
    ...
    for (const buffer of this.buffers) {
      ...
      if (jobsForBuffer?.length) {
        ...
        try {
          ...
          Logger.error(`Error encountered processing jobs in JobBuffer "${buffer.id}":\n${JSON.stringify(e.message)}`, undefined, e.stack);
        }
