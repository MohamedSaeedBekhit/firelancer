...
import { InternalServerError, JobState, notNullOrUndefined } from '../common';
  ...
  updates(options?: JobUpdateOptions): Observable<JobUpdate<T>> {
    ...
    if (!isInspectableJobQueueStrategy(strategy)) {
      ...
      return interval(pollInterval).pipe(
        ...
        takeWhile((job) => job?.state !== JobState.FAILED && job.state !== JobState.COMPLETED && job.state !== JobState.CANCELLED, true),
        tap((job) => {
